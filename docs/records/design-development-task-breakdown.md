...
- 普通怪类型：高血怪、高攻怪、均衡怪、奖励怪。
- Boss 阶段感：第 10、20、30 层先做不同奖励倾向。
- 关卡标签：金币层、经验层、装备层、Boss 层。
- 推荐挂机说明结合关卡标签和收益倾向，让玩家理解为什么推荐该层。

优先级 P2：真实构筑入口

- 小型天赋节点：暴击流、攻速流、防御流、寻宝流各 3-5 个节点。
- 构筑节点真实影响战斗、掉落或收益。
- 构筑评分保留为装备排序偏好，但不再承担全部构筑表达。

优先级 P3：经济和掉落调优

- `goldFind` 影响金币收益。
- `magicFind` 影响掉落率或高品质权重。
- 建立数值调参表，集中管理怪物成长、掉落率、金币经验曲线。
