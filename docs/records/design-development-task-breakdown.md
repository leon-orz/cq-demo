...
- [x] 构筑评分模式：`balanced`、`crit`、`speed`、`tank`、`mainAttribute`。
- [x] 推荐挂机层、推层目标、失败原因和收益倍率说明。
- [x] 最高已解锁层真实战斗胜利后解锁下一层。
- [x] `data/skills.ts`：少量被动天赋，先做暴击、攻速、坦克三条路线。
- [x] `stores/player.ts`：天赋点、激活节点、主属性方向。
- [x] `components/skilltree/SkillPanel.vue`、`SkillNode.vue`。
...
当前连续开发主线详见 `../plans/continuous-development-roadmap-2026-05-09.md`。“构筑选择 -> 推层目标 -> 掉落反馈”第一版已完成，P0 稳定化结果详见 `p0-stabilization-review-2026-05-11.md`。

优先级 P0：稳定化基线（已完成）

- 离线待领取报告持久化，避免 `稍后查看` 导致报告丢失。
- 清理历史战力和收益倍率字段，统一到派生展示口径。
- Boss 层第一版属性、收益和掉落差异已落地。
- 推荐挂机评分已纳入掉落价值期望。
- 当前测试基线为 29 个测试文件、125 条测试通过。

优先级 P1：内容厚度

- [x] 普通怪类型：高血怪、高攻怪、均衡怪、奖励怪。
- [x] Boss 阶段感：第 10、20、30 层先做不同奖励倾向。
- [x] 关卡标签：金币层、经验层、装备层、Boss 层。
- [x] 推荐挂机说明结合关卡标签和收益倾向，让玩家理解为什么推荐该层。

优先级 P2：真实构筑入口

- [x] 小型天赋节点：暴击流、攻速流、防御流、寻宝流各 3-5 个节点。
- [x] 构筑节点真实影响战斗、掉落或收益属性。
- [x] 构筑评分保留为装备排序偏好，但不再承担全部构筑表达。

优先级 P3：经济和掉落调优

- [x] `goldFind` 影响金币收益。
- [x] `magicFind` 影响掉落率或高品质权重。
- [x] 建立数值调参表，集中管理怪物成长、掉落率、金币经验曲线。

长期扩展

- 完整天赋树、强化高等级、传说特效扩展、远古装备、套装、转生、多职业、排行榜。
- 完整 GSAP 动效、Howler 音效和更严格的离线收益防作弊策略。
