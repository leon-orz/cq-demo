...
当前连续开发主线详见 `../plans/continuous-development-roadmap-2026-05-09.md`。近期任务按“构筑选择 -> 推层目标 -> 掉落反馈”推进。

优先级 P0：正式可玩核心目标

- 构筑评分系统。
  - 新增 `balanced`、`crit`、`speed`、`tank` 评分模式。
  - 让更优高亮、装备对比、只看更优和分解保护使用同一评分偏好。
  - 补核心测试，验证不同评分模式下装备排序不同。
- 推层目标系统。
  - 展示推荐挂机层、挑战层和最高解锁层。
  - 战斗失败时说明主要差距。
  - 推层成功给出明确反馈。
...
- 基础内容差异：每 10 层轻量 Boss 标记、阶段怪物差异和奖励差异。
- 组件交互测试：装备对比、分解确认、筛选面板、存档导入确认。
- 资源路径常量和占位目录：建立正式资源替换边界，但不做全量美术替换。
- 简化版天赋和强化系统评估：只在核心目标清晰后推进。
