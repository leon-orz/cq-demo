...
- [ ] 扩展战斗类型定义，新增怪物类型、关卡标签和收益倾向字段。
- [ ] 重构 `createMonster()` 和 `getStageConfig()`，从层数派生高血怪、高攻怪、均衡怪、奖励怪和 Boss 层。
- [ ] 为第 10、20、30 层 Boss 设计不同奖励倾向，初版分别偏金币、经验、装备。
- [ ] 让关卡标签进入 `StageTargetEvaluation`，推荐挂机说明结合标签、收益倾向和每秒收益输出可读原因。
- [ ] 调整中间战斗面板，展示当前层标签、怪物类型和推荐挂机原因。
- [ ] 补充核心测试，覆盖普通怪类型轮换、Boss 奖励倾向、关卡标签和推荐说明。
- [ ] 运行 `npm run format:check`、`npm run typecheck`、`npm run test:run`。
...
- 普通层不再全部显示为同一种 `腐化行者`。
- 高血怪、高攻怪、均衡怪、奖励怪至少在属性或收益上有可测试差异。
- 第 10、20、30 层 Boss 有不同奖励倾向，并能通过关卡标签和推荐说明体现。
- 推荐挂机层展示明确原因，玩家能理解它偏金币、经验、装备或 Boss 收益。
- 核心逻辑测试覆盖新字段和关键派生规则。
- 格式检查、类型检查和单元测试通过。
